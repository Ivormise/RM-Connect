...
RadioMaster Configurator - это портативное Electron-приложение, которое позволяет настраивать устройства RadioMaster T8L и возможно другие, если они появятся, через последовательный порт. Приложение работает полностью оффлайн и не требует подключения к интернету.
Для подключения T8L нужно его включить в режиме VCP. Для этого нужно держа кнопку "М" нажать кнопку включения, дождаться полного включения и после этого отпустить сначала кнопку питания, а потом кнопку "М". Пульт издаст звуковой сигнал и после этого можно будет подключать его к компьютеру.
## Возможности
